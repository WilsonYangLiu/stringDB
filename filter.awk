...
gzip -cd 9606.protein.aliases.v10.txt.gz | grep 'ENSG' | awk -F "\t" '{print $3}' | sort | uniq 
# Output: 
...
gzip -cd 9606.protein.aliases.v10.txt.gz | grep '\b[Ee]nsembl\b' | awk -F "\t" '{print $3}' | sort | \
awk '
BEGIN{FS="\s";OFS="\t";
   a="Ensembl";na=0;
...
# get Mapping of Eesemble gene id and STRING protein id
gzip -cd 9606.protein.aliases.v10.txt.gz | grep 'ENSG.*\b[Ee]nsembl\b' | \
awk '
BEGIN{FS="\t";OFS="\t"};
{
   split($1,arr,".");
...
gzip -cd 9606.protein.links.v10.txt.gz | awk '
BEGIN{FS=" ";OFS="\t"};
(NR>1){
if($3>=900){
   split($1,arr1,".");
